...
class composite_VAE(Legacy_mix_in, Overview_mix_in, Intervention_mix_in, ae1d):
    ...
    def loss(self, batch, objective=False):
        ...
        KLD = -0.5 * torch.mean(1 + logvar - mu.pow(2) - logvar.exp()) #<- bug
...
#        self.log('global_step', self.global_step)
        wkl = self.get_KL_weight()

        labels_loss = torch.nanmean(self.labels_loss(sample, y))
        ...
        if objective:
            ...
            return reconst_loss + KLD + labels_loss,\
                   reconst_loss, KLD, wkl, labels_loss
        else:
            return reconst_loss + wkl*KLD + self.lambd_labels*labels_loss,\
                   reconst_loss, KLD, wkl, labels_loss
    ...
    def training_step(self, batch, batch_idx):
        #self.memory_print()
        loss, loss_rec, kld, _, loss_label = self.loss(batch)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        loss, loss_rec, kld, _, loss_label = self.loss(
    ...
    def validation_epoch_end(self, validation_step_outputs):
        if (self.current_epoch) != 0 and (self.current_epoch  % self.overview_epoch == 0):
            ...
            for i in range(len(self.trainer.val_dataloaders)):
                ...
                if dataset_name == 'ETC':
                    spectra_idxs = [1, 10, 44, 60,100 ,200]
                else:
                    spectra_idxs = [1, 10, 44, 60,100 ,200]
                self.radvel_overview(dataset, dataset_name, spectra_idxs, self.labels)
