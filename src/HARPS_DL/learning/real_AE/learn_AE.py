...
from HARPS_DL.datasets.Spectra_real_data_module import Spectra_real_data_module as Spectra_data_module
#from HARPS_DL.tools.gpu_check import get_empty_gpu
...
def cli_main():
    ...
    cli.trainer.callbacks.append(
        ModelCheckpoint(
                monitor='real_val_loss_label',
                mode='min',
                #dirpath='checkpoints',  # Use Neptune logger's local logging directory
                #filename="{epoch}-{step}-{real_val_loss_label:.2f}",
                save_top_k=3,
                verbose=True,
            ))
